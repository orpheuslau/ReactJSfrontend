...
*/
  ...
  const fetchStudents = async () => {
    try {
      const result2 = await axios.get('https://api2.orpheuslau.dev/api/roles')
      const tempname = (await result2.data.username)

      if ((await result2.data.role) === `Admin`) { //process for Admin
        setIsAdmin(true)
        const result = await axios.get('https://api2.orpheuslau.dev/api/users')
        setUsers(await result.data);
        setAllusername(await result.data.username);
      }
      else {//process for non-admin
        const resulttemp = await axios.get('https://api2.orpheuslau.dev/api/users')
  ...
  const updateStudent = async () => {
    ...
    if (!data.name || !data.classid || !data.password || !data.role) {
      ...
      try {
        id = data._id;
        await axios.put(`https://api2.orpheuslau.dev/api/users/${id}`, data);
  ...
  const addStudent = async () => {
    if (!data.name || !data.classid || !data.password || !data.role) {
      ...
      try {
        ...
        await axios.post(`https://api2.orpheuslau.dev/api/users`, data);
  ...
  const deleteStudent = async () => {
    ...
    if (window.confirm(`Are you sure you want to delete the profile of : "${data.name}"?`)) {
      ...
      try {
        id = data._id;
        await axios.delete(`https://api2.orpheuslau.dev/api/users/${id}`, data);
